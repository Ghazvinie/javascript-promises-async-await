...
describe('Module 4', () => {
    ...
    if (checkFileExists('index')) {
      ...
      const fetchMoviesvariableDeclaration = res.body.filter(
        node =>
          ...
          _.get(node, 'declaration.declarations[0].id.name', '') ===
            'fetchMovies',
      )[0];
  ...
  it('should use the Fetch API inside of the body of `fetchMovies()` @use-fetch-api-to-get-movies', () => {
    let serviceFileExists = checkFileExists('services');
    if (serviceFileExists) {
      ...
      const body = _.has(fetchMoviesNode, 'declaration.declarations')
        ? _.get(
            fetchMoviesNode,
            'declaration.declarations[0].init.body.body[0]',
          )
    ...
    if (checkFileExists('index')) {
      ...
      if (varDec) {
        ...
        expect(
          _.get(
            body,
            'argument.callee.object.callee.object.callee.object.name',
          ),
        ).to.equal('Promise', 'You should use `Promise.all(...)`');
        expect(
          _.get(
            body,
            'argument.callee.object.callee.object.callee.property.name',
          ),
